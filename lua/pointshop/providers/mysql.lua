...
function PROVIDER:GetData(ply, callback)
    ...
    qs = string.format(qs, db:escape(ply:UniqueID()))
    local q = db:query(qs)
...
function PROVIDER:SetPoints(ply, points)
    ...
    qs = string.format(qs, db:escape(ply:UniqueID()), tonumber(points) or 0)
    local q = db:query(qs)
...
function PROVIDER:GiveItem(ply, item_id, data)
    ...
    qs = string.format(qs, db:escape(ply:UniqueID()), db:escape(util.TableToJSON(tmp)))
    local q = db:query(qs)
...
function PROVIDER:SetData(ply, points, items)
    ...
    qs = string.format(qs, db:escape(ply:UniqueID()), tonumber(points) or 0, db:escape(util.TableToJSON(items)))
    local q = db:query(qs)
